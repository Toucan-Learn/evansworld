...
test('chicken quick start supplies usable starter gear',()=>{run('startGame(true)');assert.equal(run('money'),250);assert.equal(run('level'),6);assert.equal(run('damagePerPunch()'),5);});
test('field cash bags pay more than coop bags',()=>{assert.ok(run('LEVELS[6].reward')>run('LEVELS[5].reward'));run("level=7;buildLevel(7);money=0;resolveBag(bags.find(b=>b.type==='money'))");assert.ok(run('money')>=264 && run('money')<=336);});
...
test('challenge exit works without claiming cash',()=>{run("startGame(true);level=9;buildLevel(9);player.x=portalOut.x;update(.016)");assert.equal(run('state'),'shop');assert.equal(run("bags.find(b=>b.type==='jackpot').broken"),false);run('nextLevel()');assert.equal(run('level'),10);});
test('challenge bomb positions and contents remain fixed on retries',()=>{run('startGame(true);level=9;buildLevel(9);saveCheckpoint()');const layout=run('JSON.stringify(bags)');for(let i=0;i<10;i++){run('restartLevel()');assert.equal(run('JSON.stringify(bags)'),layout);}run("resolveBag(bags.find(b=>b.type==='jackpot'))");assert.equal(run('money'),50250);});
...
test('retry restores banked checkpoint and prevents loot farming',()=>{run('startGame(true);money=3200;saveCheckpoint();money+=50000;strength+=100;die("test");restartLevel()');assert.equal(run('money'),3200);assert.equal(run('strength'),10);});
...
test('peck hits ground and can be dodged by jumping',()=>{run("buildLevel(10);player.x=tyson.x-100;tyson.st='peck';tyson.t=.2;tyson.hitDone=false;health=100;updateChickenBoss(.016)");assert.equal(run('health'),64);run("health=100;player.y=GROUND-110;tyson.hitDone=false;updateChickenBoss(.016)");assert.equal(run('health'),100);});
...
test('death loses only current-level cash after banking and shop spending',()=>{
 run("startGame(true);money=1000;resolveBag(bags.find(b=>b.type==='money'));openShop();");
...
test('old saves retain progress but clamp overpowered upgrades and disable scanner',()=>{run('startGame(true);const old={...snapshot(),strength:500,gloveLevel:90,shoeLevel:5,jumpLevel:3,armorLevel:5,scanner:true};restoreSnapshot(old)');assert.equal(run('strength'),50);assert.equal(run('gloveLevel'),5);assert.equal(run('shoeLevel'),2);assert.equal(run('jumpLevel'),1);assert.equal(run('armorLevel'),2);assert.equal(run('scanner'),false);assert.equal(run('damagePerPunch()'),15);});
test('strength cannot grow past cap from bags or training',()=>{run("startGame(true);strength=49;resolveBag(bags.find(b=>b.type==='strength'));money=10000;");assert.equal(run('strength'),50);assert.equal(run("extraShopItems().find(x=>x.t==='Power Training').can"),false);assert.equal(run("extraShopItems().some(x=>x.t.includes('Scanner'))"),false);});
