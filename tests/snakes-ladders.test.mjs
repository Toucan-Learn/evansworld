...
test('correct multiplication moves by sum and alternates players',()=>{const s={...newGame(2),dice:[3,4]};const r=answer(s,12);assert.equal(r.state.positions[0],7);assert.equal(r.state.turn,1);assert.equal(r.state.dice,null);assert.strictEqual(answer(r.state,12).state,r.state);});
test('single-player keeps its turn and ladders and snakes transport correctly',()=>{const up=answer({...newGame(),dice:[1,2]},2);assert.equal(up.state.positions[0],22);assert.equal(up.state.turn,0);const down=answer({...newGame(),positions:[20,0],dice:[3,4]},12);assert.equal(down.state.positions[0],10);});
